...
-- METHOD 1 : Using Exceptions: just run the DROP TABLE Statement and suppress the errors
...
-- METHOD 2 : Using a count variable and IF statement
DECLARE cnt NUMBER;
BEGIN
...
-- NOTE: If the table you want to delete is referenced by another you need to use the two methods
-- sequentially(method 1 then method 2) to be successful, otherwise you must drop first the foreign keys defined one by one
-- or use ON DELETE CASCADE method.
...
INSERT INTO Student (RegNo, Stud_Name, DateOfBirth, Residence) VALUES (819, 'LAURA MUSENYA','09-19/1999','KITUI');

INSERT INTO Student (RegNo, Stud_Name, DateOfBirth, Residence) VALUES (812, 'RANDOLPH KITILA', 'APR-20-1998','NAIROBI');
		
INSERT INTO Student (RegNo, Stud_Name, DateOfBirth, Residence) VALUES (814, 'MARY WAMBU', 'MAR-24-1997', 'THIKA');
...
-- The INSERT statements below won't run since CHECK Constraint is violated. DOB should be < JAN-01-2001       
...
INSERT INTO Student (RegNo, DateOfBirth, Stud_Name, Residence) VALUES (820, 'AUG-08-2000', 'SHARON GARESI','THIKA');

-- Error report -ORA-02290: check constraint (MULONGOCHELOTI.CHECK_DATE) violated
-- Solution: Drop OR Alter the constraint then repeat the insert commands
ALTER TABLE Student DROP CONSTRAINT CHECK_DATE;
-- run the three insert statements again
...
-- SOLUTION 3; Since we can't edit a constraint, we just drop the constraint FKID and create another with the necessary adjustments
...
BEGIN
    EXECUTE IMMEDIATE 'DROP VIEW Age_view'; 
    EXECUTE IMMEDIATE 'DROP VIEW Age_view_1';
    EXECUTE IMMEDIATE 'DROP VIEW Age_view_2';
    EXCEPTION
        WHEN OTHERS THEN NULL;
END;
/
...
DELETE FROM Student WHERE Stud_Name = 'LAURA MUSENYA';
...
ALTER TABLE Student
	ADD Age NUMBER GENERATED ALWAYS AS (2022 - EXTRACT(YEAR FROM DateOfBirth))VIRTUAL;
...
SELECT TO_DATE('2003/07/09', 'YYYY/MM/DD') AS modifiedDate FROM Dual;
SELECT DateOfBirth, TO_CHAR(DateOfBirth, 'DD MON, YYYY') AS mod1Date, TO_CHAR(DateOfBirth, 'DD-MON-YYYY') AS mod2Date FROM STUDENT;
